...
> **⚠️ 当前实现状态：本文档为限流设计规格，对应代码尚未开始编写。** `@RateLimit` 注解、`RateLimitService`、Redis/Caffeine 回退逻辑、Lua 脚本均为零实现。Sentinel 通过 Spring Cloud Alibaba BOM 间接引入，但 `sentinel-datasource-nacos` 和 Caffeine 依赖未在任何模块的 `pom.xml` 中显式声明。Nacos 中 `rate-limit-thresholds.yml` 已推送但无对应 `@ConfigurationProperties` 消费。以下内容作为开发时的限流实现参考。
...
> - [DEVELOPMENT-STANDARDS.md §4.2](../dev-test/DEVELOPMENT-STANDARDS.md#42-错误码清单) — 429 错误码定义
> - [ARCHITECTURE.md §6.1](../product-design/ARCHITECTURE.md#61-filter-chain-顺序) — Filter Chain 中的限流位置
