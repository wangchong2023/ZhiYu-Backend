...
### 5.1 标准发布（当前 kubeadm 环境）

```
1. Woodpecker CI 触发构建（main 分支 push）
   ├── mvn test (单元测试 + JaCoCo 覆盖率)
   ├── mvn package (构建 JAR)
   ├── docker build -f Dockerfile.kubeadm (构建镜像)
   └── Trivy 镜像扫描

2. 部署到 kubeadm 节点 (10.211.55.4)
   ├── SSH → deploy.sh kubeadm all
   ├── E2E 冒烟测试 (/actuator/health)
   └── 健康检查验证
```

### 5.2 未来发布流程（阿里云 ACK 规划）
...
> **注意：** Loki 日志聚合已在文档中设计，但当前 kubeadm 部署环境未安装 Loki。生产部署前需完成 Loki 安装与配置。当前日志通过 `kubectl logs` 直接查看。
...
- [CI-CD.md](CI-CD.md) — CI/CD 流水线与 K8s 部署清单
- [DEPLOYMENT.md](DEPLOYMENT.md) — 部署指南与离线打包
- [INFRASTRUCTURE.md](INFRASTRUCTURE.md) — Nacos 配置、Redis Key 规范
- [RATE-LIMITING.md](RATE-LIMITING.md) — 接口限流阈值配置
- [ARCHITECTURE.md](../product-design/ARCHITECTURE.md) — 部署拓扑与容灾设计
