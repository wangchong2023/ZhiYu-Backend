...
| Namespace | 用途 | 环境隔离 | 部署模式 |
|-----------|------|:------:|------|
| `public` | 保留（Nacos 默认） | — | 不使用 |
| `dev` | kubeadm 开发环境 | ✅ | 单机，外部 MySQL（与生产一致） |
...
开发者提出变更
  ...
  │     2. 验证行为正确
  │     3. 提交 PR: 更新本仓库 deploy/nacos-config/ 中的配置参考文件
...
| 环境 | 模式 | 节点数 | 持久化 | 说明 |
|------|------|:-----:|:------:|------|
| `kubeadm` | StatefulSet | 1 | ✅ AOF, 256MB `allkeys-lru` | 本机单节点开发，轻量配置 |
...
```yaml
# Redis 内存配置
# kubeadm (开发): maxmemory 256mb, maxmemory-policy allkeys-lru
# release (生产): maxmemory 4gb, maxmemory-policy volatile-lru
maxmemory: 4gb
maxmemory-policy: volatile-lru  # 仅淘汰设有 TTL 的 key（生产环境）
```
...
MySQL 的 DDL、索引、主从拓扑已在 [DATABASE.md](../product-design/DATABASE.md) 和 [ARCHITECTURE.md §5](../product-design/ARCHITECTURE.md#5-部署架构) 中定义，本节仅补充连接池配置。

**数据库列表**：

| 数据库 | 用途 | Schema 来源 |
|--------|------|------------|
| `zhiyu` | 业务主库（用户、订阅、订单、通知模板等） | V1.0.0 ~ V1.3.0 |
| `zhiyu` | 业务主库（用户设备/设置/订阅/订单/通知模板等） | V1.0.0 ~ V1.3.0 |
| `ufp_auth` | UFP 统一基础平台认证授权 | V1.4.0（auth_user, auth_role, auth_res, auth_grant 等 18 张表） |

两个数据库部署在同一 MySQL 实例，使用相同账号密码，通过 `MYSQL_DATABASE` 和 `UFP_AUTH_DATABASE` 区分。
...
> **引用索引**：
> - [DATABASE.md](../product-design/DATABASE.md) — 完整 DDL + 索引
> - [CI-CD.md §5](CI-CD.md#5-k8s-部署清单) — K8s 部署清单
> - [RATE-LIMITING.md §3](RATE-LIMITING.md#3-redis-key-设计) — 限流相关 Redis Key
> - [DEVELOPMENT-STANDARDS.md §2](../dev-test/DEVELOPMENT-STANDARDS.md#2-maven-依赖与版本管理) — Maven 依赖版本
