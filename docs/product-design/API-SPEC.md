...
所有接口返回统一 envelope（详见 [DEVELOPMENT-STANDARDS.md](../dev-test/DEVELOPMENT-STANDARDS.md#4-api-响应格式规范)）：
...
完整错误码定义见 [DEVELOPMENT-STANDARDS.md](../dev-test/DEVELOPMENT-STANDARDS.md#5-错误码体系)。各接口可能返回的错误码见每节末尾。
...
### CAPTCHA 验证码设计（P0 核心依赖）

> **⚠️ CAPTCHA 是 P0 模块（注册、登录、忘记密码、重置密码）的阻塞依赖，必须在 zhiyu-auth 开发初期落实。**

**选型：阿里云滑块验证码（ALIYUN_SLIDE）**

| 维度 | 说明 |
|------|------|
| 产品 | 阿里云验证码 2.0（滑块验证） |
| SDK | `com.aliyun:aliyun-java-sdk-core`（已在父 POM 声明） |
| 验证流程 | 前端获取 slider ticket → 后端调用阿里云 VerifyIntelligentCaptcha API 校验 |
| 配置 | `AccessKeyId` + `AccessKeySecret` 环境变量注入（不走 Nacos） |
| 降级 | 开发环境可临时使用固定 token 白名单（`zhiyu.security.captcha.skip=true`） |

**校验流程：**
```
Client                          Backend                       Aliyun
  │                                │                              │
  │  1. GET /auth/captcha/config   │                              │
  │  ← {sceneId, appKey}          │                              │
  │                                │                              │
  │  2. 前端渲染滑块                │                              │
  │  (Aliyun CAPTCHA SDK)          │                              │
  ...
  │     {captchaToken, ...}        │                              │
  │                                │  4. VerifyIntelligentCaptcha │
  │                                │  ← {result: true/false}      │
  │                                │                              │
  │  5. ← BizException(40111)      │                              │
  │     if CAPTCHA failed          │                              │
```
...
|--------|------|
| `40111` | CAPTCHA 验证未通过 |
| `40112` | CAPTCHA token 已过期（5 分钟有效） |
| `40113` | CAPTCHA 服务不可用（跳过开关开启时忽略） |
...
zhiyu:
  security:
    captcha:
      skip: true  # 开发环境跳过 CAPTCHA 校验，接受任意 token
```
> 生产环境 `captcha.skip=false`，由 K8s Secret 注入阿里云 AK/SK。

---

## 1. 认证接口 `/auth/**`

> **优先级标注：** `[P0]` = MVP 第一版必须 | `[P1]` = 第一轮迭代 | `[P2]` = 迭代二

### 1.0 获取 CAPTCHA 配置 `[P0]`
...
| 字段 | 类型 | 说明 |
|------|------|------|
| sceneId | string | 阿里云 CAPTCHA 场景标识 |
| appKey | string | 阿里云 CAPTCHA 应用 Key（前端 SDK 初始化用） |
| captchaType | string | 验证码类型，当前固定 `ALIYUN_SLIDE` |

**说明：** 前端先调用此接口获取 sceneId 和 appKey，再初始化阿里云 CAPTCHA SDK 渲染滑块。开发环境 `zhiyu.security.captcha.skip=true` 时返回 mock 数据。
...
**可能错误：** 40001 参数校验 | 40110 验证码发送频率过高（60s 冷却） | 40111 CAPTCHA 未通过 | 42903 IP 限流（1/min per email, 20/day per email）
...
> **优先级标注：** `[P0]` = MVP 第一版必须 | `[P1]` = 第一轮迭代 | `[P2]` = 迭代二
>
> P0 仅需 `3.1`（套餐列表）+ `3.2`（当前订阅状态），其余为 P1/P2。
...
> **优先级标注：** `[P0]` = MVP 第一版必须 | `[P1]` = 第一轮迭代 | `[P2]` = 迭代二
>
> P0 仅需 `4.1`（管理员登录+TOTP）+ `4.2`（用户列表/详情/禁用）+ `4.3`（审计日志查看），其余为 P1/P2。
...
> **优先级：** `[P2]` — 文件上传依赖 OSS 和配额体系，第一版不实现。
