...
# 内容见 docs/INFRASTRUCTURE.md §1.4.1

# Data ID: subscription-plans.yml, Group: SUBSCRIPTION, Format: YAML
# 内容见 docs/INFRASTRUCTURE.md §1.4.2
```
...
├── docs/                              # 全部设计文档
├── zhiyu-common/                      # 公共模块（工具/异常/安全 Filter/i18n）
...
> 完整开发流程（规划 → TDD → 代码审查 → 提交）见全局规则 `~/.claude/rules/common/development-workflow.md`。
...
├── zhiyu-common/                    # 公共模块 — 零业务依赖
├── zhiyu-auth/                      # 认证模块
├── zhiyu-user/                      # 用户模块
├── zhiyu-subscription/              # 订阅/支付模块
├── zhiyu-admin/                     # 管理后台模块
└── zhiyu-server/                    # 可运行入口（装配 + 启动）
```
...
`zhiyu-common` 不依赖任何业务模块。上层可依赖下层（通过 Service 接口注入），同层不互相依赖。禁止跨模块直接注入 Mapper。
...
**依赖：** 零业务依赖。仅依赖 Spring Boot Starter Web、MyBatis-Plus、jjwt、Spring Security Crypto。
...
**依赖：** `zhiyu-common` + `spring-boot-starter-data-redis`
...
<project>
    ...
    <dependencyManagement>
        <dependencies>
            ...
            <dependency>
                <groupId>com.zhiyu</groupId>
                <artifactId>zhiyu-common</artifactId>
                <version>${revision}</version>
            </dependency>
            <!-- ...其他内部模块同理... -->
        </dependencies>
...
> 更多缓存策略细节见 [INFRASTRUCTURE.md §2.6-2.7](../INFRASTRUCTURE.md)。
...
> Outbox 表 DDL 见 [DATABASE.md §3.9](../DATABASE.md#39-异步事件)，由定时 Job 轮询发送，保证通知至少发送一次（At-Least-Once），下游需做幂等。
...
B - 领域分类：
    0=通用  1=认证  2=授权  3=用户  4=订阅/支付
    5=资源  6=第三方服务  7=限流/熔断  8=系统内部
CDE - 具体错误（001-999）
```
...
2. 确认测试失败          → ./gradlew test --tests "XxxTest"
3. 写最小实现（GREEN）    → 只写够让测试通过的代码
4. 确认测试通过          → ./gradlew test
5. 重构（IMPROVE）       → 消除重复、改善命名、提取方法
6. 跑全量测试确认无回归   → ./gradlew test
```
...
| 代码风格 | Checkstyle | ESLint + Prettier |
| 静态分析 | SpotBugs | — |
...
> **引用规范**: 本文档遵循全局规则 [coding-style.md](~/.claude/rules/common/coding-style.md) 和 [testing.md](~/.claude/rules/common/testing.md)，所有 Java 通用约定以全局规则为准，本文档仅补充 ZhiYu 项目特有的 Spring Boot / MyBatis-Plus / React 规范。
