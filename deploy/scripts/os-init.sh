...
# 脚本名称: os-init.sh
# 脚本功能: 专职负责物理机器（无网或有网）宿主机系统级基础环境与核心依赖软件的一键开荒自愈安装。
#           本脚本具备以下企业级开荒特性：
#             - 智能双模支持：支持 `--offline`（离线自适应，物理加载 deploy/packages/ 缓存底座）与 
#               `--online`（在线实时拉取）双模式。
#             - 完备的平台兼容性：内嵌对 macOS (Homebrew)、Ubuntu (22.04+)、Debian (12+) 的自动辨识与兼容。
#             - 细颗粒度自愈模块：自动化完成宿主机 Docker 容器引擎、kubectl 命令行客户端、
#               OpenJDK 17 运行环境、Maven 编译环境、envsubst、envoy、Argo Rollouts CLI 
#               等运维与开发底层栈的一键式零人工干预安装。
#             - 智能干运行模式：支持 `--dry-run` 用以仅检查不实际实施，方便运维预审。
# 编 写 人: 资深架构师 & 高级开发工程师 (Antigravity AI)
# 编写时间: 2026-05-20
# 用    法:
#           sudo ./deploy/scripts/os-init.sh [--offline | --online] [--dry-run] [--docker-only]
# ==============================================================================
set -euo pipefail
...
if [ "$MODE" = "offline" ] && [ ! -d "$PACKAGES_DIR" ]; then
  ...
  log_error ""
  log_error "请先在联网机器上运行离线依赖下载模块:"
  log_error "  ./deploy/scripts/download-packages.sh all"
  log_error ""
  log_error "然后打包整个 deploy/ 目录并复制到目标机器进行部署自愈即可。"
  exit 1
fi
...
# 函数名称: check_cmd
# 函数功能: 校验指定命令行工具在当前宿主机上是否已经安装，并高亮打印已安装的版本号
# 输入参数: $1 - 工具名称 (例如: docker, kubectl)
# 返 回 值: 0 - 已安装, 1 - 未安装
check_cmd() {
...
# 函数名称: need_install
# 函数功能: 探测系统是否确实需要安装某个指令。若找不到该命令，或其版本与期望版本匹配失败，则判定需要安装
# 输入参数: $1 - 工具名称, $2 - (可选) 版本正则表达式匹配串
# 返 回 值: 0 - 需要安装, 1 - 已经安装且版本满足
need_install() {
  local cmd="$1"
...
# 函数名称: run
# 函数功能: 控制指令执行的干路包装器。若开启干运行模式，则仅打印指令而不实际对系统做物理改动
# 输入参数: $@ - 待运行的完整命令行及参数
# 返 回 值: 级联返回被执行命令的退出码
run() {
  if $DRY_RUN; then
...
# 函数名称: install_from_local
# 函数功能: 专用于离线模式下，从 deploy/packages/ 缓存归档中一键解析并物理安装软件
# 输入参数: $1 - 包分类目录 (例如: linux/amd64/tools), 
#           $2 - 通配匹配模式 (例如: helm-v*.tar.gz), 
#           $3 - 目标安装物理目录 (默认: /usr/local/bin), 
#           $4 - 压缩包解压时需要剥离的前置目录层数 (默认: 0)
# 返 回 值: 0 - 安装成功, 1 - 缺失离线包报错
install_from_local() {
  local category="$1"
  local pattern="$2"
  ...
  if [ -z "$file" ]; then
    ...
    log_error "  请先运行: ./deploy/scripts/download-packages.sh"
    return 1
  fi
...
verify() {
  ...
  if check_cmd java && check_cmd docker && check_cmd kubectl; then
    ...
    echo "  2. cd \${PROJECT_DIR} && ./deploy/deploy.sh kubeadm all"
  else
